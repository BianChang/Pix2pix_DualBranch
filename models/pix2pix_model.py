...
def L2_L1_Loss(pred, mask, weight=None):
    device = torch.device('cuda:0')
    L2loss = nn.MSELoss(reduction='none')
    L1loss = nn.L1Loss()
    l2_channel_loss = L2loss(pred, mask)
    zero = torch.tensor(np.zeros(pred.shape)).to(device)
    train_loss = L1loss(l2_channel_loss, zero)
...
class Pix2PixModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        if self.isTrain:
            ...
            self.criterionL1L2 = L2_L1_Loss
            self.criterionFocal = ChannelWeightedFocalLoss(weights=torch.tensor([1.0, 2.0, 1.0]))
    ...
    def backward_G(self):
        ...
        if self.opt.loss_type == 'CE':
            self.loss_G_L1 = self.criterionCE(self.fake_B, self.real_B) * self.opt.lambda_L1
        if self.opt.loss_type == 'MSE':
            self.loss_G_L1 = self.criterionMSE(self.fake_B, self.real_B) * self.opt.lambda_L1
        if self.opt.loss_type == 'L1L2':
            self.loss_G_L1 = self.criterionL1L2(self.fake_B, self.real_B) * self.opt.lambda_L1
        if self.opt.loss_type == 'Focal':
            self.loss_G_L1 = self.criterionFocal(self.fake_B, self.real_B) * self.opt.lambda_L1
