...
def save_images(webpage, visuals, image_path, aspect_ratio=1.0, width=256, use_wandb=False):
    ...
    Parameters:
        webpage (the HTML class) -- the HTML webpage class that stores these imaegs (see html_utils.py for more details)
...
def save_images(webpage, visuals, image_path, aspect_ratio=1.0, width=256, use_wandb=False):
    ...
    for label, im_data in visuals.items():
        ...
        # Check if the image is 1-channel and handle it as grayscale
        if im.shape[2] == 1:  # 1-channel image
            im = im[:, :, 0]  # Remove the channel dimension for grayscale images
...
class Visualizer():
    ...
    def __init__(self, opt):
        ...
        self.loss_img_name = os.path.join(opt.checkpoints_dir, opt.name)
    ...
    def display_current_results(self, visuals, epoch, save_result):
        ...
        if self.use_html and (save_result or not self.saved):  # save images to an HTML file if they haven't been saved.
            ...
            # update website
            webpage = html_utils.HTML(self.web_dir, 'Experiment name = %s' % self.name, refresh=1)
    ...
    def novisdom_plot_losses(self, epoch, counter_ratio, losses, max_epoch):
        """plot loss curve when visdom_id = 0
         Parameters:
            epoch (int)           -- current epoch
            counter_ratio (float) -- progress (percentage) in the current epoch, between 0 to 1
            losses (OrderedDict)  -- training losses stored in the format of (name, float) pairs
